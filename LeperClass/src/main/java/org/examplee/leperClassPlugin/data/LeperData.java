...
public final class LeperData {
   ...
   public void setLeper(Player p, boolean val) {
      if (val) {
         p.getPersistentDataContainer().set(this.keys.leperKey, PersistentDataType.BYTE, (byte)1);
      } else {
   ...
   public int getInfectionHits(Player p) {
      return (Integer)p.getPersistentDataContainer().getOrDefault(this.keys.infectionHitsKey, PersistentDataType.INTEGER, 0);
   }

   public void setInfectionHits(Player p, int hits) {
      p.getPersistentDataContainer().set(this.keys.infectionHitsKey, PersistentDataType.INTEGER, hits);
   }

   public int getInfectionStage(Player p) {
      return (Integer)p.getPersistentDataContainer().getOrDefault(this.keys.infectionStageKey, PersistentDataType.INTEGER, 0);
   }

   public void setInfectionStage(Player p, int stage) {
      p.getPersistentDataContainer().set(this.keys.infectionStageKey, PersistentDataType.INTEGER, stage);
   }
   ...
   public void setInfectionNextPhaseMs(Player p, long ms) {
      p.getPersistentDataContainer().set(this.keys.infectionNextPhaseKey, PersistentDataType.LONG, ms);
   }
   ...
   public void setDangerBlessed(Player p, boolean blessed) {
      if (blessed) {
         p.getPersistentDataContainer().set(this.keys.dangerBlessKey, PersistentDataType.BYTE, (byte)1);
      } else {
   ...
   public void setRageUntil(Player p, long ms) {
      p.getPersistentDataContainer().set(this.keys.rageUntilKey, PersistentDataType.LONG, ms);
   }
}
