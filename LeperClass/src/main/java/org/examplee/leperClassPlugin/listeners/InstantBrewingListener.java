...
public final class InstantBrewingListener implements Listener {
   ...
   public void onClick(InventoryClickEvent e) {
      if (e.getInventory() instanceof BrewerInventory inv) {
         this.plugin.getServer().getScheduler().runTask(this.plugin, () -> this.handle(inv));
      }
   }
   ...
   private boolean convert(BrewerInventory inv, int mode) {
      ...
      for (int slot = 0; slot < 3; slot++) {
         ...
         if (cur != null && cur.getType() != Material.AIR) {
            switch (mode) {
               case 0:
                  if (this.plugin.tags.isLeperBlood(cur)) {
                     inv.setItem(slot, this.plugin.items.makeThickLeperBlood());
                     changed = true;
                  }
                  break;
               case 1:
                  if (this.plugin.tags.isThickLeperBlood(cur)) {
                     inv.setItem(slot, this.plugin.items.makeSterileLeperBlood());
                     changed = true;
                  }
                  break;
               case 2:
                  if (this.plugin.tags.isSterileLeperBlood(cur)) {
