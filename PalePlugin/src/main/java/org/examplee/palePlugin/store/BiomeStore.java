...
public final class BiomeStore {
   ...
   public void restoreCellsIfClean(World world, Set<Long> cellKeys, Set<Material> infectedTypes, Biome infectedBiome) {
      for (long key : cellKeys) {
         ...
         if (oldKey != null) {
            ...
            if (!this.cellHasInfected(world, xCell, yCell, zCell, infectedTypes)) {
               Biome oldBiome = (Biome)Registry.BIOME.get(oldKey);
               if (oldBiome == null || infectedBiome != null && oldBiome.equals(infectedBiome)) {
                  oldBiome = (Biome)Registry.BIOME.get(NamespacedKey.fromString("minecraft:plains"));
               }
      ...
      for (int ox = 0; ox < 4; ox++) {
         ...
         for (int oz = 0; oz < 4; oz++) {
            ...
            for (int oy = 0; oy < 4; oy++) {
               int y = baseY + oy;
               if (y >= minY && y <= maxY && infectedTypes.contains(world.getBlockAt(x, y, z).getType())) {
                  return true;
               }
            }
         }
   ...
   public List<String> serialize() {
      ArrayList<String> out = new ArrayList<>(this.oldByCell.size());

      for (Entry<Long, NamespacedKey> e : this.oldByCell.entrySet()) {
         long key = e.getKey();
   ...
   public void loadSerializedLine(String s) {
      ...
      if (p.length == 4) {
         try {
            ...
            NamespacedKey key = NamespacedKey.fromString(p[3].toLowerCase(Locale.ROOT));
            if (key == null) {
               return;
            }
